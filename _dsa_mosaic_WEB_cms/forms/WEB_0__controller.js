	...
	for (var i = 1; i <= obj.version.record.web_version_to_area.getSize(); i++) {
		...
		for (var h = 1; h <= rows.getSize(); h++) {
			...
			if (obj.type == 'Edit' && utils.hasRecords(area.web_area_to_editable) && area.web_area_to_editable.flag_new_block) {
				...
				var breadcrumb = 'Add block to ' + globals.CODE_text_initial_caps(area.area_name) + ' : ' + globals.CODE_text_initial_caps(row.row_name)
				newBlock += '<div id="sutra-block-add-' + rowString + '" class="block_new">'
				newBlock += '<a href="javascript:blockNew(\'' + rowString + '\')" title="' + breadcrumb + '">' + breadcrumb + '</a>'
				newBlock += '</div>'
		...
		if (obj.type == 'Edit' && utils.hasRecords(area.web_area_to_editable) && area.web_area_to_editable.flag_new_block) {
			...
			breadcrumb = 'Add row to ' + globals.CODE_text_initial_caps(area.area_name)
			newRow += '<div id="sutra-row-add-' + areaString + '" class="row_new">\n'
			newRow += '<a href="javascript:rowNew(\'' + areaString + '\')" title="' + breadcrumb + '">' + breadcrumb + '</a>'
			newRow += '</div>\n'
